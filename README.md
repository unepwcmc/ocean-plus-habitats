...
## ⚠️ Troubleshootings
1. Some warning / error "failed" messages can appear and be logged in the files `failed_report_countries.csv` and `failed_report_regions.csv` stored in `~/ocean-plus-habitats/current/tmp`, we can copy these files from the server to our local machine thanks to this command:  
