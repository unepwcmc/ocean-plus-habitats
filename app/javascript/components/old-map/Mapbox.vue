<template>
  <div class="map--statistics sm-counter-trigger">
    <div class="map__panel gutters">
      <p class="map__panel-title">
        {{ titleGlobal }}
      </p>

      <p class="map__panel-stat no-margin">
        <counter
          v-if="percentageGlobal"
          sm-trigger="sm-counter-trigger"
          sm-target="sm-counter-target"
          :number="percentageGlobal"
        />
        <template v-if="habitatType != 'points'">
          km<sup>2</sup>
        </template>
      </p>
      ...
      <span class="map__panel-stat">
        <counter
          v-if="percentageProtected"
          sm-trigger="sm-counter-trigger"
          sm-target="sm-counter-target"
          :number="percentageProtected"
        />%
      </span>

      <p class="map__panel-layer map__panel-layer-habitat">
        {{ habitatTitle }}
      </p>
      <p class="map__panel-layer">
        Protected Areas
      </p>
    </div>
